...
RULES: dict[str, list[Rule]] = {
    "attack": [
        ...
        Rule(r"\bdestroy(?:s|ed|ing)?\b|\bannihilat(?:e|es|ed|ing)\b|破壊|壊滅|滅ぼ", 5, "destruction expression"),
        Rule(r"\benergy wave\b|\bbeam\b|\blaser(?:s)?\b|\bkamehameha\b|ビーム|レーザー|エネルギー波|かめはめ波", 4, "energy attack expression"),
        Rule(r"\bmissile(?:s)?\b|\brepulsor(?:s)?\b|\bweapon(?:s)?\b|\boffensive systems?\b|武器|兵器|銃|剣|刀", 4, "weapon expression"),
        Rule(r"\brasengan\b|\battack(?:s)?\b|攻撃|必殺技|螺旋丸", 4, "attack technique expression"),
        Rule(r"\bfighting skills?\b|\bmartial arts?\b|格闘|武術", 3, "combat skill expression"),
        Rule(r"\bbattle(?:s|d|ing)?\b|\bfight(?:s|ing)?\b|戦闘|戦う|戦い|戦士", 2, "battle/fight expression"),
    ],
    "defense": [
    ...
    "speed": [
        ...
        Rule(r"\bflight\b|\bfly\b|\bflies\b|飛行|空を飛", 4, "flight expression"),
        Rule(r"\bfast\b|\bspeed\b|\bquick\b|速度|高速|素早", 3, "speed expression"),
    ],
    "abilities": [
        Rule(r"\bsuperhuman\b|超人的", 5, "superhuman expression"),
        Rule(r"\bdemon fox\b|\bnine-tailed\b|九尾|悪魔|魔人", 5, "sealed entity expression"),
        Rule(r"\btechnique(?:s)?\b|\bjutsu\b|\brasengan\b|\bkamehameha\b|技|術|忍術|魔法|能力", 4, "technique expression"),
        Rule(r"\bnanotechnology\b|ナノテク", 4, "technology expression"),
        Rule(r"\bcapable of\b|\bable to\b|\bcan\b|できる|可能|使う", 3, "capability expression"),
        Rule(r"\bpowered exoskeleton(?:s)?\b|\bweapon(?:s)?\b|装備|武器|兵器", 3, "equipment expression"),
        ...
        Rule(r"\bbecomes\b|\btransform(?:s|ed|ing)?\b|変身|変化|形態", 3, "transformation expression"),
    ],
    "feats": [
        Rule(r"\bdefeat(?:s|ed|ing)?\b|\bwin(?:s|ning)?\b|\bvictor(?:y|ies|ious)?\b|倒す|倒した|撃破|勝利", 5, "victory expression"),
        Rule(r"\bprotect(?:s|ed|ing)?\b|\bsav(?:e|es|ed|ing)\b|守る|救う|救出|保護", 3, "protection/saving feat"),
        Rule(r"\bbattle(?:s|d|ing)?\b|\bfight(?:s|ed)?\b|戦闘|戦う|戦い", 3, "battle feat"),
    ...
    "iq": [
        ...
        Rule(r"\binvent(?:or|s|ed|ion)?\b|\bengineer(?:s|ed|ing)?\b|\bscientist\b|発明|科学者|研究者", 4, "invention/science expression"),
        Rule(r"\bstrateg(?:y|ist|ic|ical)\b|\btactic(?:s|al)?\b|戦略|戦術|策略", 4, "strategy/tactics expression"),
        Rule(r"\bdetective abilities\b|\bdetective\b|探偵", 3, "detective expression"),
        Rule(r"\bintellect\b|\bintellectual(?:ly)?\b|知性|知能|知的|頭脳", 3, "intellect expression"),
